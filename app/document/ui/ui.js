...
function open_reference_image() {
  ...
  if (files) {
    $(
      "reference_image"
    ).style.backgroundImage = `url(${electron.nativeImage
      .createFromPath(files[0])
      .toDataURL()})`;
...
function rescale_guide() {
  $("guide").style.width = `${doc.render.font.width *
    Math.min(doc.columns, guide_columns)}px`;
  $("guide").style.height = `${doc.render.font.height *
    Math.min(doc.rows, guide_rows)}px`;
...
function set_zoom(factor) {
  ...
  send("update_menu_checkboxes", {
    actual_size: electron.remote.getCurrentWebContents().getZoomFactor() == 1
  });
}
...
class Tools extends events.EventEmitter {
  ...
  start(new_mode) {
    ...
    if (this.mode != undefined)
      this.get_tool_div(this.mode).classList.remove("selected");
  ...
  constructor() {
    ...
    document.addEventListener("DOMContentLoaded", (event) => {
      ...
      $("rectangle_mode").addEventListener(
        "mousedown",
        (event) => this.start(this.modes.RECTANGLE),
        true
      );
      $("ellipse_mode").addEventListener(
        "mousedown",
        (event) => this.start(this.modes.ELLIPSE),
        true
      );
  ...
  change_mode(new_mode) {
    ...
    switch (this.mode) {
      ...
      case this.modes.SHADING_BLOCK:
        ...
        break;
      case this.modes.CLEAR_BLOCK:
        $("clear_block").classList.add("brush_mode_selected");
        break;
  ...
  constructor() {
    ...
    document.addEventListener(
      ...
      (event) => {
        ...
        $("clear_block").addEventListener("mousedown", (event) =>
          this.change_mode(this.modes.CLEAR_BLOCK)
        );
