...
const help_menu_items = {
  ...
  submenu: [
    ...
    {
      label: "Mœbius Homepage",
      id: "show_homepage",
      click(item) {
        electron.shell.openExternal("http://www.andyh.org/moebius/");
      }
    },
    {
    ...
    {
      label: "Raise an Issue at GitHub",
      id: "show_issues",
      click(item) {
        electron.shell.openExternal(
          "https://github.com/blocktronics/moebius/issues"
        );
      }
    },
    { type: "separator" },
    ...
    submenu: [
      {
        ...
        click(item) {
          event.emit("show_new_connection_window");
        }
      }
    ]
  },
    ...
    submenu: [
      ...
      chat
        ...
        : {
            label: "Redo",
            id: "redo",
            accelerator: darwin ? "Cmd+Shift+Z" : "",
            click(item) {
              win.send("redo");
            },
            enabled: false
      ...
      { type: "separator" },
      {
        ...
        id: "toggle_insert_mode",
        accelerator: darwin ? "" : "Insert",
        type: "checkbox",
        click(item) {
          win.send("insert_mode", item.checked);
        },
        checked: false
      },
      {
        label: "Toggle Overwrite Mode",
        id: "overwrite_mode",
        accelerator: "CmdorCtrl+Alt+O",
        click(item) {
          win.send("overwrite_mode", item.checked);
        },
        type: "checkbox",
        checked: false
      },
      { type: "separator" },
      {
        ...
        click(item) {
          win.send("mirror_mode", item.checked);
        },
        type: "checkbox",
        checked: false
      },
  ...
  return {
    ...
    submenu: [
      ...
      { type: "separator" },
      {
        ...
        accelerator: "T",
        click(item) {
          win.send("transparent", item.checked);
        },
        type: "checkbox",
        checked: false,
        enabled: false
      ...
      },
      {
        ...
        accelerator: "U",
        click(item) {
          win.send("underneath", item.checked);
        },
        type: "checkbox",
        checked: false,
        enabled: false
...
function font_menu_items(win) {
  return Object.keys(font_list).map((menu_title) => {
    return {
      label: menu_title,
      submenu: Object.keys(font_list[menu_title]).map((font_name) => {
        return {
  ...
  return {
    ...
    submenu: [
      ...
      { type: "separator" },
      {
        ...
        accelerator: "CmdorCtrl+/",
        click(item) {
          win.send("show_statusbar", item.checked);
        },
        type: "checkbox",
        checked: true
      },
      ...
      },
      {
        ...
        click(item) {
          win.send("show_preview", item.checked);
        },
        type: "checkbox",
        checked: true
      },
      ...
      { type: "separator" },
      {
        ...
        click(item) {
          win.send("use_9px_font", item.checked);
        },
        type: "checkbox",
        checked: false
      },
      ...
      { type: "separator" },
      {
        ...
        submenu: [
          {
            label: "Smallscale (80×25)",
            id: "smallscale_guide",
            click(item) {
              win.send("toggle_smallscale_guide", item.checked);
            },
            type: "checkbox",
            checked: false
          },
          {
            ...
            id: "square_guide",
            click(item) {
              win.send("toggle_square_guide", item.checked);
            },
            type: "checkbox",
            checked: false
          },
          {
            label: "Instagram (80×50)",
            id: "instagram_guide",
            click(item) {
              win.send("toggle_instagram_guide", item.checked);
            },
            type: "checkbox",
            checked: false
          },
          {
            ...
            id: "file_id_guide",
            click(item) {
              win.send("toggle_file_id_guide", item.checked);
            },
            type: "checkbox",
            checked: false
          }
      ...
      {
        label: "Toggle Reference Image",
        id: "toggle_reference_image",
        accelerator: "Ctrl+Tab",
        click(item) {
          win.send("toggle_reference_image", item.checked);
        },
        enabled: false,
        type: "checkbox",
        checked: true
      ...
      { type: "separator" },
      {
        label: "Scroll Document With Cursor",
        id: "scroll_document_with_cursor",
        accelerator: "CmdorCtrl+R",
        click(item) {
          win.send("scroll_document_with_cursor", item.checked);
        },
        type: "checkbox",
        checked: false
      },
...
function colors_menu_template(win) {
  return {
    ...
    submenu: [
      ...
      { type: "separator" },
      {
        label: "Use iCE Colors",
        id: "ice_colors",
        accelerator: "CmdorCtrl+E",
        click(item) {
          win.send("ice_colors", item.checked);
        },
        type: "checkbox",
        checked: false
      }
  ...
  return {
    ...
    submenu: [
      ...
      { type: "separator" },
      {
        ...
        id: "chat_window_toggle",
        accelerator: "CmdorCtrl+[",
        click(item) {
          win.send("chat_window_toggle");
        },
        enabled
      }
    ]
...
function create_menu_template(win, chat, debug) {
  const menu_lists = [
    file_menu_template(win),
    edit_menu_template(win, chat),
    selection_menu_template(win, chat),
    colors_menu_template(win),
    view_menu_template(win),
    network_menu_template(win, chat)
  ];
...
function enable(id, name) {
  ...
  if (
    ...
    get_chat_menu_item(id, name).enabled = true;
}
...
function disable(id, name) {
  ...
  if (
    ...
    get_chat_menu_item(id, name).enabled = false;
}
...
electron.ipcMain.on("disable_selection_menu_items", (event, { id }) =>
...
electron.ipcMain.on("disable_operation_menu_items", (event, { id }) =>
...
class MenuEvent extends events.EventEmitter {
  ...
  chat_input_menu(win, debug) {
    const menu = darwin
      ? electron.Menu.buildFromTemplate([
          moebius_menu,
          ...create_menu_template(win, true, debug),
          window_menu_items,
          help_menu_items
        ])
      : electron.Menu.buildFromTemplate([
  ...
  document_menu(win, debug) {
    const menu = darwin
      ? electron.Menu.buildFromTemplate([
          moebius_menu,
          ...create_menu_template(win, false, debug),
          window_menu_items,
          help_menu_items
        ])
      : electron.Menu.buildFromTemplate([
